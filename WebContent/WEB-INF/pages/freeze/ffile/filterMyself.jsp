...
<div class="quick-action">
	<a href="action/ffile/viewAddMyself" class="button-like"><span class="add-ffile">发布新文件</span></a>
</div>
<div class="box">
	...
	<c:choose>
		...
		<c:otherwise>
			<form action="action/ffile/batchDeleteMyself">
				<!-- 批量删除 -->
				...
				<table class="dataTable">
					...
					<tr>
						...
						<td>
							...
							<a href="javascript:void(0);" id="action/ffile/deleteMyself?id=${ffile.id }" class="delete jqModal">删除</a>
						</td>
					</tr>
					</c:forEach>
				</table>
				...
				<div id="pageBar">
					<div id="barL" class="l">
						...
						<c:choose>
							...
							<a href="action/ffile/filterMyself?by=${by }&order=${order }&page=1&countPerPage=${pager.countPerPage}"
						...
						<c:choose>
							<c:when test="${pager.currentPage > 1 }">
								<a href="action/ffile/filterMyself?by=${by }&order=${order }&page=${pager.currentPage - 1 }&countPerPage=${pager.countPerPage}"
							...
							<c:when
								...
								<c:forEach var="p" begin="1"
									...
									<c:choose>
										...
										<c:otherwise>
											<a href="action/ffile/filterMyself?by=${by }&order=${order }&page=${p }&countPerPage=${pager.countPerPage}"
						...
						<c:choose>
							<c:when test="${pager.currentPage < pager.totalPage }">
								<a href="action/ffile/filterMyself?by=${by }&order=${order }&page=${pager.totalPage}&countPerPage=${pager.countPerPage}"
					...
					<div id="barR" class="r">
						...
						<input type="button" value="GO" class="go-bt" id="action/ffile/filterMyself?by=${by }&order=${order }&page=${pager.currentPage}${queryCondition}"/>
					</div>
					<div class="clear"></div>
				</div>
				<!-- 分页 end -->
