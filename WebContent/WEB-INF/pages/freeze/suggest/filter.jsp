...
<body>
	...
	<div class="crumb">
		<div class="addsuggest-title">反馈</div>
		<div class="backNav">
	...
	<div class="box">
		...
		<c:choose>
			...
			<c:otherwise>
				<div class="actionTip">温馨提醒：<span class="tip-words">以下为所有用户反馈</span></div>
				<!-- 分页条 start -->
				<div id="pageBar">
					<div id="barL" class="l">
						<!-- 搜索式分页 -->
						...
						<c:choose>
							<c:when test="${pager.currentPage > 1 }">
								<a
									href="action/suggest/filter?by=${by }&order=${order }&page=1&countPerPage=${pager.countPerPage}"
						...
						<c:choose>
							<c:when test="${pager.currentPage > 1 }">
								<a
									href="action/suggest/filter?by=${by }&order=${order }&page=${pager.currentPage - 1 }&countPerPage=${pager.countPerPage}"
							...
							<c:when
								...
								<c:forEach var="p" begin="1" end="${maxPagerShowLength }"
									...
									<c:choose>
										...
										<c:otherwise>
											<a
												href="action/suggest/filter?by=${by }&order=${order }&page=${p }&countPerPage=${pager.countPerPage}"
						...
						<c:choose>
							<c:when test="${pager.currentPage < pager.totalPage }">
								<a
									href="action/suggest/filter?by=${by }&order=${order }&page=${pager.totalPage}&countPerPage=${pager.countPerPage}"
					...
					<div id="barR" class="r">
						${pager.currentPage }/${pager.totalPage }页，共${pager.totalCount}条 <select
							...
							id="action/suggest/filter?by=${by }&order=${order }&page=${pager.currentPage}${queryCondition}" />
					</div>
					<div class="clear"></div>
				</div>
				<!-- 分页 end -->
				<table class="dataTable">
					...
					<c:forEach var="suggest" items="${dataList }">
						<tr>
							<td><a href="action/suggest/show?id=${suggest.id }" title="查看此反馈详细"
								class="detail-suggest"> <c:choose>
							...
							<td>
								<fmt:formatDate value="${suggest.addtime }" type="both" />
							</td>
							<td>
								<c:choose>
